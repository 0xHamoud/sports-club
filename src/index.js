...
const MASTER_CODE = '0502777888';

let PINS = {
  رجال:   bcrypt.hashSync('1234', 10),
  سيدات: bcrypt.hashSync('1234', 10),
};
...
app.post('/api/auth/change-pin', async (req, res) => {
  ...
  if (masterCode !== MASTER_CODE) return res.status(401).json({ error: 'رمز التغيير خاطئ' });
...
app.listen(PORT, () => console.log(`✅ السيرفر شغّال على http://localhost:${PORT}`));
